    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <div className="flex gap-3 justify-center">
          <Link href="/orders" className="btn-blue px-6 py-3">
            View Orders
          </Link>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <h1 className="font-display font-bold text-2xl text-ink mb-8">
        Your Cart{" "}
        <span className="text-ink-faint text-lg font-sans font-normal">
          ({items.length} items)
        </span>
      </h1>
          ...
          {items.map((item) => (
            <div key={item._id} className="card p-4 flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-display font-semibold text-ink text-sm line-clamp-1 mt-0.5">
                  {item.name}
                </h3>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-1 bg-surface2 border border-border rounded-lg p-1">
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center text-ink-muted hover:text-ink transition-colors text-lg"
                    >
                      −
                    </button>
                    <span className="text-ink text-sm font-medium w-6 text-center">
                      {item.qty}
                    </span>
                    <button
                      onClick={() =>
        ...
        <div className="card p-6 h-fit relative overflow-hidden">
          ...
          {step === "address" && (
            <div className="flex flex-col gap-3 mb-6">
              <p className="text-xs font-mono font-semibold text-ink-faint uppercase tracking-wider">
                Delivery Address
              </p>
              <input
          ...
          <div className="flex flex-col gap-2 text-sm mb-6">
            ...
            <div className="flex justify-between text-ink-muted">
              <span>Payment</span>
              <span className="text-brand font-medium">Stripe (Test Mode)</span>
            </div>
          ...
          {step === "address" && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-100 rounded-xl">
              <p className="text-xs font-mono font-semibold text-brand mb-2">
                🧪 Stripe Test Cards
              </p>
